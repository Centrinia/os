...
struct LispValue * parse_expression(std::string::iterator & oit, const std::string::iterator & end) {
    ...
    if((out = parse_atom(it=oit, end)) != NULL) {
        oit = it;
        return out;
    } else if((out = parse_string(it=oit, end)) != NULL) {
        oit = it;
        return out;
    } else if((out = parse_number(it=oit, end)) != NULL) {
        oit = it;
        return out;
    } else if(*oit == '(') {
        it=oit;
    ...
    } else if((out = parse_quoted(it = oit,end)) != NULL) {
        oit = it;
        return out;
    }
