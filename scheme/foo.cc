...
# 59 "/usr/include/c++/4.8/bits/stl_algobase.h" 3
...
# 60 "/usr/include/c++/4.8/bits/stl_algobase.h" 2 3
...
struct LispValue * parse_expression(std::string::iterator & oit, const std::string::iterator & end) {
    ...
    if((out = parse_atom(it=oit, end)) != __null) {
        oit = it;
        return out;
    } else if((out = parse_string(it=oit, end)) != __null) {
        oit = it;
        return out;
    } else if((out = parse_number(it=oit, end)) != __null) {
        oit = it;
        return out;
    } else if(*oit == '(') {
        it=oit;
    ...
    } else if((out = parse_quoted(it = oit,end)) != __null) {
        oit = it;
        return out;
    }
...
# 519 "scheme.cc"
int int_add(int a, int b) { return a + b; } struct LispValue * add( std::vector<struct LispValue*>::const_iterator & it, const std::vector<struct LispValue*>::const_iterator & end) { return fold_binop(int_add,it,end); }
int int_sub(int a, int b) { return a - b; } struct LispValue * sub( std::vector<struct LispValue*>::const_iterator & it, const std::vector<struct LispValue*>::const_iterator & end) { return fold_binop(int_sub,it,end); }
# 555 "scheme.cc"
}
...
int main() {
    for(;;) {
        ...
        if((vp = parse_expression(it, str.end())) != __null) {
            std::cout << "Parsed: " << *vp << std::endl;
            if((ep = evaluate_expression(*vp)) != __null) {
