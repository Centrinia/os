...
#define PAGE_TABLE_0_ADDRESS	((640*1024-4096*2) & (-4096))
...
static void enable_paging(void * page_directory)
{
 ...
 uint32_t eax = 0;
 /* Enable paging. */
 asm volatile("mov %%cr0,%0\n\tor %1,%0\n\tmov %0,%%cr0" : : "a" (eax),"i" (1<<31));
 /* Enable 4MB pages. */
 asm volatile("mov %%cr4,%0\n\tor %1,%0\n\tmov %0,%%cr4" : : "a" (eax),"i" (1<<4));

}
void setup_paging() {
	struct page_directory_page * page_directory = (struct page_directory_page*)PAGE_DIRECTORY_ADDRESS;
	...
	struct page_directory_table * page_tables = (struct page_directory_table*)PAGE_DIRECTORY_ADDRESS;
	...
	struct page_table_entry * page_table = (struct page_table_entry*)PAGE_TABLE_0_ADDRESS;
	for(int i=0;i<1024;i++) {
